...
function getCategoryReferenceFromCategoryTree(
  ...
  if (categoryTree.categoryId === categoryId) {
    console.log('FOUND', categoryTree.categoryName)
    selected = categoryTree
  } else if (categoryTree.children.length > 0) {
    for (let i = 0; i < categoryTree.children.length; i++) {
      const childCategory = categoryTree.children[i]
      selected = getCategoryReferenceFromCategoryTree(childCategory, categoryId)
